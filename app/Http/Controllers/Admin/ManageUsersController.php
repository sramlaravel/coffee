...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $product['totalProduct'] = Product::where('user_id', $user->id)->count();
        $product['purchase'] = Order::where('status', '!=', 0)->where('user_id', $user->id)->count();
        $product['soldProduct'] = Order::whereHas('product', function($q) use ($user){
            $q->where('user_id', $user->id);
        })->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdraw','totalTransaction','countries', 'product'));
    }

    public function userProduct($id)
    {   
       $user = User::findOrFail($id);
       $pageTitle  = ucfirst($user->username) . ' all product list';
       $emptyMessage = "No data found";
       $categorys = Category::where('status', 1)->select('id', 'name')->get(); 
       $products = Product::where('user_id', $user->id)->latest()->with('user', 'category', 'subCategory')->paginate(getPaginate());
       return view('admin.product.index', compact('pageTitle', 'emptyMessage', 'products', 'categorys')) ;
    }

    public function userSoldProduct($id)
    {
        $user = User::findOrFail($id);
        $pageTitle = ucfirst($user->username) . ' sold product list';
        $emptyMessage = "No data found";
        $orders = Order::whereHas('product', function($q) use ($user){
            $q->where('user_id', $user->id);
        })->where('status', '!=', 0)->with('user', 'product', 'product.user')->paginate(getPaginate());
        return view('admin.order.index', compact('pageTitle', 'emptyMessage', 'orders'));
    }

    public function userPurchaseProduct($id)
    {
        $user = User::findOrFail($id);
        $pageTitle = ucfirst($user->username) . ' purchase product list';
        $emptyMessage = "No data found"; 
        $orders = Order::where('user_id', $user->id)->where('status', '!=', 0)->with('user', 'product', 'product.user')->paginate(getPaginate());
        return view('admin.order.index', compact('pageTitle', 'emptyMessage', 'orders'));
    }
